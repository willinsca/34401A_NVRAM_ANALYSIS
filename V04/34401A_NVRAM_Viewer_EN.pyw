...
APP_TITLE = "34401A NVRAM Viewer v0.4 English + Dump Reader"
SCALE_Q23 = 8_388_608
...
class NvramImage:
    ...
    def overview_text(self) -> str:
        ...
        lines = [
            "34401A NVRAM Viewer v0.4 English — READ ONLY ANALYSIS + DUMP READER",
            "=" * 68,
            f"File: {self.path}",
...
class NvramViewer(tk.Tk):
    ...
    def _build_cal_assistant_tab(self) -> None:
        ...
        ttk.Button(value_frame, text="Use selected option", command=self.use_selected_value_option).grid(row=0, column=2, padx=(8, 0))
        ttk.Label(
            controls,
            text=(
                "Option 1 fills the nominal full-scale template. The primary/compare dump options deliberately show/copy "
                "the historical raw Q23 record instead of placing it in CAL:VAL, because no valid one-to-one conversion "
                "from EEPROM correction data to an external calibration-standard value has been established."
            ),
            wraplength=1000,
    ...
    def refresh_value_source_choices(self) -> None:
        """Populate the visible CAL-value choices for the selected range."""
        spec = self.selected_spec()
        choices = [
            f"Nominal full-scale template: {spec.nominal_example}",
            "Primary dump record: show/copy historical Q23 data (not CAL:VAL)",
        ]
        if self.compare_image is not None:
            choices.append("Comparison dump record: show/copy historical Q23 data (not CAL:VAL)")
    ...
    def use_selected_value_option(self) -> None:
        ...
        self.refresh_dump_record_reference()
        self.status_var.set(
            "Selected dump record shown below and available to copy. It was not inserted into CAL:VAL because "
            "an EEPROM correction record is not an external calibration-standard value."
        )
    ...
    def export_csv(self) -> None:
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as handle:
                ...
                writer.writerow(["34401A NVRAM Viewer v0.4 English — READ ONLY ANALYSIS + DUMP READER export"])
