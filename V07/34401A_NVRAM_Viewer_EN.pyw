...
APP_TITLE = "34401A NVRAM Viewer v0.7 English + SCPI Command Console"
SCALE_Q23 = 8_388_608
...
class NvramImage:
    ...
    def overview_text(self) -> str:
        ...
        lines = [
            "34401A NVRAM Viewer v0.7 English — ANALYSIS + SCPI COMMAND CONSOLE",
            "=" * 68,
            f"File: {self.path}",
...
class NvramViewer(tk.Tk):
    ...
    def _build_calstr_tab(self) -> None:
        intro = ttk.Label(
            ...
            text=(
                ...
                "The local Test *IDN? button uses the same serial open / send / 300 ms wait / CR-line read path as Read dump from 34401A."
            ),
            justify="left",
            padding=(10, 10, 10, 4),
            wraplength=1360,
        ...
        helper = ttk.LabelFrame(self.calstr_tab, text="CAL:STR template helper", padding=10)
        helper.pack(fill="x", padx=8, pady=4)
        helper.columnconfigure(1, weight=1)

        ttk.Label(helper, text="String to write:").grid(row=0, column=0, sticky="w", padx=(0, 8), pady=3)
        self.calstr_entry = ttk.Entry(helper, textvariable=self.calstr_text_var, width=70)
        self.calstr_entry.grid(row=0, column=1, sticky="w", pady=3)
        ttk.Button(helper, text="Use primary dump string", command=lambda: self.load_calstr_from_dump(False)).grid(row=0, column=2, padx=(8, 0), pady=3)
        ttk.Button(helper, text="Use comparison dump string", command=lambda: self.load_calstr_from_dump(True)).grid(row=0, column=3, padx=(8, 0), pady=3)

        ttk.Label(
            helper,
            text="Printable ASCII only, maximum 40 characters. The helper inserts a single-quoted CAL:STR literal into the editable script.",
            wraplength=1080,
        ).grid(row=1, column=1, columnspan=3, sticky="w", pady=(0, 6))

        ttk.Label(helper, text="Security code (not stored):").grid(row=2, column=0, sticky="w", padx=(0, 8), pady=3)
        ttk.Entry(helper, textvariable=self.calstr_security_code_var, width=28).grid(row=2, column=1, sticky="w", pady=3)

        ttk.Checkbutton(
            helper,
            text="Include CAL:SEC:STAT OFF in generated template",
            variable=self.calstr_unlock_var,
        ).grid(row=3, column=1, sticky="w", pady=3)
        ttk.Checkbutton(
            helper,
            text="Append CAL:SEC:STAT ON in generated template",
            variable=self.calstr_resecure_var,
        ).grid(row=4, column=1, sticky="w", pady=3)

        helper_actions = ttk.Frame(helper)
        helper_actions.grid(row=0, column=4, rowspan=5, sticky="ns", padx=(15, 0))
        ttk.Button(helper_actions, text="Load CAL:STR template", command=self.generate_calstr_commands).pack(fill="x", pady=(0, 6))
        ttk.Button(helper_actions, text="Load CAL Assistant block", command=self.load_cal_assistant_block).pack(fill="x", pady=(0, 6))
        ttk.Button(helper_actions, text="Copy script", command=self.copy_console_script).pack(fill="x")

        status_frame = ttk.LabelFrame(self.calstr_tab, text="Template status", padding=8)
        ...
        self.console_editor = ScrolledText(console_frame, wrap="none", height=12, font=("Consolas", 10))
        self.console_editor.pack(fill="both", expand=True, pady=(6, 0))
        self.console_editor.bind("<Control-a>", self.select_all_text)

        live_frame = ttk.LabelFrame(self.calstr_tab, text="Live serial runner", padding=10)
        ...
        ttk.Button(live_frame, text="Test *IDN?", command=self.start_console_idn_test).grid(row=0, column=3, padx=(8, 0), pady=3)
        ...
        ttk.Label(live_frame, text="Delay after write:").grid(row=2, column=0, sticky="w", padx=(0, 8), pady=3)
        ttk.Entry(live_frame, textvariable=self.console_delay_ms_var, width=8).grid(row=2, column=1, sticky="w", pady=3)
        ttk.Label(live_frame, text="ms").grid(row=2, column=1, sticky="w", padx=(65, 0), pady=3)
        ttk.Checkbutton(
            live_frame,
        ...
        ).grid(row=3, column=1, sticky="w", pady=(8, 3))
        ttk.Label(
            live_frame,
            text="A confirmation dialog shows the exact executable lines before sending them.",
        ).grid(row=3, column=2, columnspan=2, sticky="w", padx=(12, 0), pady=(8, 3))
    ...
    def load_calstr_from_dump(self, use_comparison: bool) -> None:
        ...
        self.generate_calstr_commands()
        self.status_var.set(f"Loaded calibration string from {label} dump and placed a generated template into the editor.")
    ...
    def build_calstr_template_lines(self) -> List[str]:
        ...
        if self.calstr_unlock_var.get():
            if not code_value:
                raise ValueError("Security code is empty while the generated template is set to include an unsecure command.")
        ...
        if self.calstr_resecure_var.get():
            if not code_value:
                raise ValueError("Security code is empty while the generated template is set to include a secure command.")
            lines.extend([
    ...
    def generate_calstr_commands(self) -> None:
        try:
            ...
            self.calstr_status_var.set(
                ...
                "CAL:STR template loaded into the editable command script."
            )
            if hasattr(self, "console_editor"):
    ...
    def start_console_idn_test(self) -> None:
        """Local *IDN? test using the same read-dump serial path and timings."""
        if not PYSERIAL_AVAILABLE:
        ...
        if not device:
            messagebox.showerror("No COM port selected", "Click Refresh ports and select the port connected to the 34401A.")
            return

        terminator = self.selected_line_ending()
        self.dump_running = True
        self.dump_abort_event.clear()
        self.calstr_execute_status_var.set("Testing *IDN? from the SCPI Command Console…")
        self.append_calstr_log(f"[INFO] Console *IDN? test requested on {device}.")
        threading.Thread(target=self.console_idn_worker, args=(device, terminator), daemon=True).start()

    def console_idn_worker(self, device: str, terminator: bytes) -> None:
        """Same open/send/300ms/read sequence as Read dump from 34401A."""
        handle = None
        try:
            handle = self.serial_open(device)
            self.after(0, self.append_calstr_log, f"[INFO] Opened {device}: 9600 8N2 DTR/DSR")
            self.after(0, self.append_calstr_log, ">> *IDN?")
            self.serial_send(handle, "*IDN?", terminator)
            time.sleep(0.300)  # exactly matches the read-dump *IDN? worker delay
            ...
            detail = f"Console *IDN? test failed: {exc}"
    ...
    def export_csv(self) -> None:
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as handle:
                ...
                writer.writerow(["34401A NVRAM Viewer v0.7 English — ANALYSIS + SCPI COMMAND CONSOLE export"])
