...
It can generate copyable, user-filled SCPI CAL and CAL:STR command templates.
...
APP_TITLE = "34401A NVRAM Viewer v0.5 English + Dump Reader + CAL:STR Assistant"
SCALE_Q23 = 8_388_608
...
class NvramImage:
    ...
    def overview_text(self) -> str:
        ...
        lines = [
            "34401A NVRAM Viewer v0.5 English — ANALYSIS + READ-ONLY DUMP READER",
            "=" * 68,
            f"File: {self.path}",
...
class NvramViewer(tk.Tk):
    ...
    def _build_toolbar(self) -> None:
        ...
        note = ttk.Label(
            frame,
            text="Viewer + read-only serial dump: no SCPI calibration transmission and no EEPROM write.",
        )
    ...
    def _build_cal_assistant_tab(self) -> None:
        ...
        ttk.Label(
            controls,
            text=(
                "Nominal template fills a numeric formatting example. Selecting a primary/compare dump updates the "
                "migration-reference panel and the generated command to a non-sendable CAL:VAL placeholder. "
                "The EEPROM record does not contain a proven actual external standard value, so the program will not "
                "manufacture a false number for a real calibration command."
            ),
            wraplength=1000,
    ...
    def _build_calstr_tab(self) -> None:
        intro = ttk.Label(
            self.calstr_tab,
            text=(
                "This assistant generates a copyable CAL:STR write / verify sequence. "
                "It does not transmit commands itself. The command block uses the same unlocked-calibration workflow "
                "you already tested: query security state, optionally unsecure, write the string, query it back, and check the error queue."
            ),
            justify="left",
            padding=(10, 10, 10, 4),
            wraplength=1360,
        ...
        controls = ttk.LabelFrame(self.calstr_tab, text="Calibration string template", padding=10)
        ...
        ttk.Checkbutton(
            controls,
            text="Include CAL:SEC:STAT OFF before writing",
            variable=self.calstr_unlock_var,
        ).grid(row=3, column=1, sticky="w", pady=3)
        ttk.Checkbutton(
            controls,
            text="Append CAL:SEC:STAT ON after verification",
            variable=self.calstr_resecure_var,
        ).grid(row=4, column=1, sticky="w", pady=3)

        actions = ttk.Frame(controls)
        actions.grid(row=0, column=4, rowspan=5, sticky="ns", padx=(15, 0))
        ttk.Button(actions, text="Generate / refresh", command=self.generate_calstr_commands).pack(fill="x", pady=(0, 6))
        ttk.Button(actions, text="Copy command block", command=self.copy_calstr_commands).pack(fill="x")
        ...
        self.calstr_output = ScrolledText(output_frame, wrap="none", height=16, font=("Consolas", 10))
    ...
    def refresh_value_source_choices(self) -> None:
        """Populate the visible source choices for the selected range."""
        spec = self.selected_spec()
        primary_model = q23_modeled_fullscale_value(self.image, spec)
        primary_detail = (
            f"Primary dump modeled value: {primary_model:.12g} {spec.unit} (reference only)"
            if primary_model is not None
            else "Primary dump modeled value: no primary dump loaded"
        )
        choices = [
        ...
        if self.compare_image is not None:
            ...
            choices.append(
                f"Comparison dump modeled value: {compare_model:.12g} {spec.unit} (reference only)"
                if compare_model is not None
                else "Comparison dump modeled value: no comparison dump loaded"
            )
        self.assistant_value_source_combo.configure(values=choices)
        current = self.assistant_value_source_var.get()
    ...
    def use_selected_value_option(self) -> None:
        ...
        # The user asked for the selected source to visibly alter the Actual Applied
        # Standard field and generated command. It does so with an explicit non-sendable
        # placeholder rather than silently using an unproven conversion.
        self.assistant_actual_value_var.set("<DUMP_VALUE_IS_REFERENCE_ONLY>")
        self.generate_assistant_commands()
        modeled_text = (
            f"{modeled:.12g} {spec.unit}" if modeled is not None else "unavailable"
        )
        self.status_var.set(
            f"{source_name} selected. Candidate modeled full-scale value = {modeled_text}. "
            "It is displayed/copyable as a migration reference, but not used as CAL:VAL."
        )
    ...
    def generate_assistant_commands(self) -> None:
        ...
        if actual_text == "<DUMP_VALUE_IS_REFERENCE_ONLY>":
            self.assistant_warning_var.set(
                "A dump record was selected. Its modeled value is a migration reference, not a proven external standard value. "
                "The command block is intentionally non-sendable until you enter a certified actual source value."
            )
            value_for_command = "<EXTERNAL_STANDARD_VALUE_REQUIRED>"
        elif actual_text and not self.valid_decimal_text(actual_text):
    ...
    def refresh_dump_record_reference(self) -> None:
        ...
        # Show the other loaded record too, whenever it exists, so the primary and
        # compare dumps can be assessed side by side without mixing them into CAL:VAL.
        ...
        text += (
            "\\n\\nWhy this is not CAL:VAL:\\n"
            "The EEPROM record is an internal correction result. CAL:VAL requires the "
            "actual external standard signal applied during a new calibration. The raw "
            "gain/offset pair does not encode the source's certified output or the "
            "meter's uncorrected live reading, so no one-to-one conversion is known."
        )
    ...
    def export_csv(self) -> None:
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as handle:
                ...
                writer.writerow(["34401A NVRAM Viewer v0.5 English — ANALYSIS + READ-ONLY DUMP READER export"])
    ...
    def show_about(self) -> None:
        messagebox.showinfo(
            ...
            "• Separate copyable CAL:STR write / verify template generator\n"
