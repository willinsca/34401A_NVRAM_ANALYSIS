...
APP_TITLE = "34401A NVRAM Viewer v0.10 English + CAL Defaults & Gate"
SCALE_Q23 = 8_388_608
...
class CalibrationStringInfo:
    """Decoded view of the physical packed text region W[32]..W[4A]."""
    raw_chars: Tuple[int, ...]
    raw_visible: str
    layout: str
    header: str
...
def decode_cal_string_info(words: List[int]) -> CalibrationStringInfo:
    """
    Decode the physical 7-bit packed region:
      - begins at W[32] bit 0;
      - each 16-bit word is LSB-first;
      - each character is 7-bit LSB-first.

    Two layouts are recognized:

    1. Legacy continuous layout:
       text begins at packed character 0 and continues until NUL.

    2. Segmented remote-CAL:STR layout observed after a remote CAL:STR write:
       characters 0..7   : retained header (often HP034401)
       characters 8..12  : five NUL separator characters
       characters 13..52 : remote CAL:STR payload, up to 40 characters

    The old viewer rendered all printable characters across the NUL separator and
    therefore visually concatenated the retained header with the remote payload.
    """
    bits: List[int] = []
    ...
    # Five NULs after the first eight characters are the observed remote-write
    # delimiter. Then 40 characters are the field populated by CAL:STR.
    separator = chars[8:13]
    segmented = len(chars) >= 53 and separator == [0, 0, 0, 0, 0]

    if segmented:
        header = _printable_until_nul(chars[0:8])
        remote_string = _printable_until_nul(chars[13:53])
        return CalibrationStringInfo(
            raw_chars=tuple(chars),
            raw_visible=raw_visible,
            layout="Segmented remote CAL:STR layout",
            header=header,
            separator_nuls=5,
    ...
    return CalibrationStringInfo(
        raw_chars=tuple(chars),
        raw_visible=raw_visible,
        layout="Legacy continuous calibration-string layout",
        header="",
        separator_nuls=0,
        remote_string="",
        legacy_string=legacy_string,
...
def decode_cal_string(words: List[int]) -> Tuple[str, str]:
    ...
    info = decode_cal_string_info(words)
...
class NvramImage:
    ...
    def overview_text(self) -> str:
        ...
        cal_info = decode_cal_string_info(self.words)
        w = self.words
        ...
        lines = [
            "34401A NVRAM Viewer v0.10 English — ANALYSIS + CAL DEFAULTS & GATE",
            "=" * 72,
            f"File: {self.path}",
        ...
        if cal_info.layout.startswith("Segmented"):
            lines.extend([
                f"  Retained header chars 0..7: {cal_info.header}",
                "  Separator chars 8..12: five NUL characters",
                f"  Remote CAL:STR field chars 13..52: {cal_info.remote_string}",
                f"  Preferred CAL:STR text: {cal_info.preferred_string}",
                "  Note: the remote CAL:STR field begins after the five-NUL separator.",
                "        The retained header is not concatenated into the preferred CAL:STR text.",
            ])
        else:
            lines.extend([
                f"  Legacy continuous text: {cal_info.legacy_string}",
                f"  Preferred CAL:STR text: {cal_info.preferred_string}",
            ])
        ...
        lines.extend([
            ...
            "  CAL:VAL may be filled from the selected dump-derived Q23 model in this application.",
            "  It remains distinct from an independently measured external calibration standard.",
        ])
        return "\n".join(lines)
...
class NvramViewer(tk.Tk):
    ...
    def _build_calstr_tab(self) -> None:
        intro = ttk.Label(
            self.calstr_tab,
            text=(
                "SCPI Command Console: generate a CAL:STR sequence, paste a command block from SCPI CAL Assistant, "
                "or write your own commands. The editor is what will be executed, line by line. "
                "The local Test *IDN? button uses the exact same transport worker as Read dump from 34401A."
            ),
            justify="left",
            padding=(10, 8, 10, 2),
            wraplength=1360,
        ...
        helper = ttk.LabelFrame(self.calstr_tab, text="CAL:STR template helper", padding=8)
        ...
        self.calstr_entry = ttk.Entry(
            helper,
            textvariable=self.calstr_text_var,
            width=54,
            validate="key",
        ...
        # These flags are intentionally independent. Both checked means:
        # OFF before writing -> CAL:STR / verify -> ON after verification.
        ttk.Checkbutton(
            helper,
            text="Unlock before write (OFF)",
            variable=self.calstr_unlock_var,
        ).grid(row=1, column=2, sticky="w", padx=(16, 0), pady=2)
        ttk.Checkbutton(
            helper,
            text="Re-secure after verify (ON)",
            variable=self.calstr_resecure_var,
        ).grid(row=1, column=3, sticky="w", padx=(14, 0), pady=2)
        ttk.Button(helper, text="Load CAL Assistant block", command=self.load_cal_assistant_block).grid(
            row=1, column=4, padx=(8, 0), pady=2
        )
        ttk.Button(helper, text="Copy script", command=self.copy_console_script).grid(
            row=1, column=5, padx=(6, 0), pady=2
        )

        ttk.Label(
            helper,
            text=(
                "The two security options are independent: select both for OFF → write/verify → ON. "
                "Select only OFF to leave the meter unsecured. Select only ON only makes sense when the meter is already unsecured."
            ),
            justify="left",
            wraplength=1320,
    ...
    def set_calstr_default_from_primary_dump(self) -> None:
        """
        Set the CAL:STR helper default from the currently opened primary dump.
        For segmented dumps, this uses the remote CAL:STR payload only and excludes
        the retained HP034401 header.
        """
        if self.image is None:
            return

        cal_info = decode_cal_string_info(self.image.words)
        self.calstr_text_var.set(cal_info.preferred_string)
        if cal_info.layout.startswith("Segmented"):
            self.calstr_source_var.set("Default remote CAL:STR field from primary dump (header excluded)")
        else:
            self.calstr_source_var.set("Default legacy CAL:STR field from primary dump")
        self.generate_calstr_commands()
        self.calstr_template_state_var.set(
            "Default loaded from the primary dump. Review or edit it, then use UPDATE TEMPLATE when needed."
        )
    ...
    def load_calstr_from_dump(self, use_comparison: bool) -> None:
        ...
        if image is None:
            ...
            return
        cal_info = decode_cal_string_info(image.words)
        self.calstr_text_var.set(cal_info.preferred_string)
        if cal_info.layout.startswith("Segmented"):
            self.calstr_source_var.set(f"Remote CAL:STR field from {label} dump (header excluded)")
        else:
            self.calstr_source_var.set(f"Legacy continuous field from {label} dump")
        self.generate_calstr_commands()
        self.status_var.set(
            f"Loaded preferred CAL:STR text from {label} dump. "
            f"Layout: {cal_info.layout}."
        )
    ...
    def generate_calstr_commands(self) -> None:
        try:
            ...
            self.update_calstr_input_status()
            self.calstr_template_state_var.set(
                "Template updated in the editor. The editor is now ready to review, edit, copy, or execute."
            )
        except ValueError as exc:
            self.calstr_status_var.set("Not ready: " + str(exc))
            self.calstr_template_state_var.set("Template was not updated because the helper input is invalid.")
    ...
    def load_cal_assistant_block(self) -> None:
        ...
        self.console_editor.delete("1.0", "end")
        self.console_editor.insert("1.0", block + ("\n" if block and not block.endswith("\n") else ""))
        self.calstr_status_var.set(
            f"Loaded SCPI CAL Assistant block for {candidate}; Actual applied standard value = {actual_value}."
        )
        self.calstr_template_state_var.set(
            "Console contains the SCPI CAL Assistant block calculated from the selected Candidate function / range."
        )
        self.status_var.set("SCPI CAL Assistant block loaded into the console editor. Review it before execution.")
    ...
    def export_csv(self) -> None:
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as handle:
                ...
                writer.writerow(["34401A NVRAM Viewer v0.10 English — ANALYSIS + CAL DEFAULTS & GATE export"])
    ...
    def show_about(self) -> None:
        messagebox.showinfo(
            ...
            "• Correct calibration-string LSB-first 7-bit decoding, including segmented remote CAL:STR field detection\n• Primary-dump CAL:STR payload loaded as the Console helper default\n"
            ...
            "• Unified SCPI Command Console: editable script runner + CAL:STR helper\n"
